...
function GlobalTasksView({ pendingTasksMap, conversations, agents, onSelectConv, onRefresh }) {
  ...
  const fetchDone = async () => {
    try {
      const r = await fetch(`${API_URL}/tasks/completed?tenant_id=${auth?.user?.tenant_id || ""}&days=7`, { headers });
      const d = await r.json();
...
function TasksPanel({ convId, agents, onClose, onTaskDone }) {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const r = await fetch(`${API_URL}/tasks?tenant_id=${auth?.user?.tenant_id || ""}&conversation_id=${convId}`, { headers });
      if (r.ok) {
        const d = await r.json();
...
function ReportsView({ auth, T = { app: "#f0f2f5", card: "#ffffff", border: "#e9edef", text: "#111b21", text2: "#667781" }, waInstances = [], defaultInstance = null }) {
  ...
  const fetchReport = async (type, inst) => {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/reports/${type}?tenant_id=${auth?.user?.tenant_id || ""}&days=${days}${instQ}`, { headers });
      const d = await r.json();
