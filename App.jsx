          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 40, maxWidth: 720 }}>
            ...
            <div style={{ background: "#0d0d18", border: "1px solid #1a1a2e", borderRadius: 14, padding: 24 }}>
              ...
              {[["Tenant ID", TENANT_ID],["API URL", API_URL],["Versão", "7zap Inbox v1.0"]].map(([label, value]) => (
                <div key={label} style={{ display: "flex", gap: 12, marginBottom: 8 }}><span style={{ fontSize: 12, color: "#555", width: 100, flexShrink: 0 }}>{label}</span><span style={{ fontSize: 12, color: "#888", fontFamily: "monospace", wordBreak: "break-all" }}>{value}</span></div>
              ))}
            </div>
          </div>
        )}
